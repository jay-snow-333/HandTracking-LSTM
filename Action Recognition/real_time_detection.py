...
# 调用摄像头
# cap = cv2.VideoCapture('output1671017760.7621222.avi')
# cap = cv2.VideoCapture('c20195b810f38ea15bf61d042a0545bd.mp4')
...
if __name__ == "__main__":
    with mpHands.Hands(max_num_hands=1,
        ...
        while True:
            ...
            # 当结果中最大值大于检测置信度时：
            hands_result['frame'].append(frame_time)
            if current_conf > detection_confidence:
                # 如果当前有手势判定结果，则和手势判定结果进行比对，不同时则出现了新的手势
                hands_result['result'].append(np.argmax(result) + 1)
                ...
                hands_result['result'].append(0)
            # 只显示最新5个检测结果
            if len(gesture_sentence) > 10:
                gesture_sentence = gesture_sentence[-7:]

            # gesture_sentence为手势识别序列结果
            seq_str = "pinch  press  pinch  insert  pinch  insert  screw  prod  screw"

            cv2.rectangle(frame, (0, 0), (640, 40), (180, 3, 77), thickness=-1)
            cv2.putText(frame, "  ".join(gesture_sentence), (3, 25),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
            cv2.putText(frame, seq_str, (3, 10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
            # cv2.putText(frame,"  ".join(gesture_sentence),(3,40),
            #             cv2.FONT_HERSHEY_SIMPLEX, 1.5, (255, 255, 255), 1, cv2.LINE_AA)

            cv2.imshow("Opencv Feed", frame)
            out.write(frame)
            print(hands_result['frame'])
            print(hands_result['result'])
            frame_time += 1
            if frame_time > 800:
                list_record = hands_result['result']
                list_pos = hands_result['frame']
                list_ = [list_pos, list_record]
                list_name = ['frame', 'result']
                result_save = pd.DataFrame(data=list_record)
                result_save.to_csv('result_save2.csv')
